...
def main():
    ...
    check_dir = CHECKPOINT_PATH + configuration + "finetune.pt"

    # Prediction Checkpoint Path
    PRED_PATH = "./preds_y/finetune_final/"
    pred_dir = PRED_PATH + configuration + "finetune.pt"
    ...
    for epoch in range(args.epochs):
        ...
        for bc, batch in enumerate(train_loader):        
            ...
            if corr.isnan():
                ...
                continue
            corr_ += corr
            sys.stdout.write('\r[ epoch {}/{} | batch {}/{} ] RMSE: {:.4f} | Corr: {:.4f} | MAE: {:.4f} '.format(epoch + 1, args.epochs, bc + 1, num_batch + 1, rmse, corr, mae))
