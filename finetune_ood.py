...
from embedder import GNTransformer
...
def test(model, data_loader, criterion, device):
    ...
    return loss/(bc + 1),loss_mae/(bc+1), preds_y
...
def main():
    ...
    configuration = utils.exp_get_name2(train_config)
    if args.OOD:
    ...
    # CHECKPOINT_PATH = "./model_checkpoints/ood/"
    # check_dir = CHECKPOINT_PATH + configuration + "ood.pt"

    # # Prediction Checkpoint Path
    # PRED_PATH = "./preds_y/ood/"
    # pred_dir = PRED_PATH + configuration + "ood.pt"
    ...
    ######################################## Dataset: Out of distribution by crystal system ############################################### 
    #######################################################################################################################################
    # dataset_train = torch.load("./data/processed/dos_ood_train.pt")
    # dataset_test = torch.load("./data/processed/dos_ood_test.pt")
    ...
    # print("Dataset Loaded!")
    # ######################################## Dataset: Out of distribution by # elements ################################################### 
    # #######################################################################################################################################
    ...
    if embedder =='gntransformer':
        model = model = GNTransformer(args.layers, args.transformer, n_atom_feat, n_bond_feat, n_glob_feat, n_hidden, dim_out, device).to(device)
    else :
        print("error occured : Inappropriate model name")
    print(model)

    f = open("./experiments/experiments.txt".format(embedder), "a")
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.eval == 0 :
            ...
            #valid
            valid_loss,valid_mae, preds_y = test(model, valid_loader, criterion_2, device)
            ...
            if valid_loss < best_loss:
                ...
                test_loss, test_mae, preds_y = test(model, test_loader, criterion_2, device)
