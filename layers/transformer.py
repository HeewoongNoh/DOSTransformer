...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x_in, x_in_k = None, x_in_v = None, mask = None):
        ...
        if x_in_k is not None and x_in_v is not None:
            ...
            x_k = F.dropout(x_in_k, p=self.dropout, training=self.training)
            x_v = F.dropout(x_in_v, p=self.dropout, training=self.training)
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, embed_dim, num_heads=4, attn_dropout=0.0, relu_dropout=0.0, res_dropout=0.0):
        super().__init__()
        self.embed_dim = embed_dim
