...
class mlp_2(nn.Module):
    def __init__(self, layers, n_atom_feats, n_bond_feats, n_glob_feats, n_hidden, dim_out, device):
        ...
        # self.out_layer = nn.Sequential(nn.Linear(n_hidden, n_hidden), nn.LayerNorm(n_hidden), nn.PReLU(), \
        #     nn.Linear(n_hidden, n_hidden//2), nn.LayerNorm(n_hidden//2), nn.PReLU(), nn.Linear(n_hidden//2, 201))
    ...
    def forward(self, g):
        ...
        # size = g.batch.max().item() + 1 if size is None else size
        
        dos_vector1 = self.out_layer(x)
        # dos_vector2 = scatter_sum(x,g.batch,dim=0)
        dos_vector3 = scatter_sum(dos_vector1, g.batch, dim=0)

        
        # dos_graph = self.out_layer(self.alpha * graph)
        # dos = dos_graph.squeeze(2).T
